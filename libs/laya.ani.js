...
var Templet=(function(_super){
	...
	//TODO:SL #region start
	/**
	* 设置骨骼路径
	*/
	__getset(0, __proto, 'skeletonUrl',function (){
		return this._skBufferUrl;
	},function (value){
		if (this._skBufferUrl != value){
			this.useBoneBufferCache = false;
		}
		this._skBufferUrl = value;
	});

	/**
	 * 清空GraphicCache
	 */
	__proto.clearGraphicCache = function (){
		this._graphicsCache.length = 0;
	}
	//TODO:SL #region end
	...
	*/
	__proto._parsePublicExtData=function(){
		...
		//TODO:SL #region start
		if (this.useBoneBufferCache)
		{
			var len;
			len = this.boneSlotArray.length;
			for (i = 0; i < len; i++){
				this.boneSlotArray[i].clearCurData();
			}

			len = this.skinSlotDisplayDataArr.length;
			for (i = 0; i < len; i++){
				this.skinSlotDisplayDataArr[i].texture = null;
			}
			...
			this.mBoneArr.length = 0;
			this.srcBoneMatrixArr.length = 0;
			this.boneSlotArray.length = 0;
			this.ikArr.length = 0;
			this.skinDataArray.length = 0;
			this.skinSlotDisplayDataArr.length = 0;
			this.drawOrderAniArr.length = 0;
			this.deformAniArr.length = 0;
			this.eventAniArr.length = 0;
		}
		//TODO:SL #region end
