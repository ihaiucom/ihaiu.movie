	...
	Timer.__init$=function(){
		...
		TimerHandler=(function(){
			...
			__proto.run=function(withClear){
				...
				// TODO ZF
				if (window["Games"] && window["Games"]["DebugTimerLoopManager"] && window["Games"]["DebugTimerLoopManager"].enable)
				{
					if (window["Games"]["DebugTimerLoopManager"].onCallerRun(caller, method))
					{
						var cost = new Date().getTime();
						args ? method.apply(caller, args) : method.call(caller);
						cost = new Date().getTime() - cost;
						window["Games"]["DebugTimerLoopManager"].setCallerCost(caller, method, cost)
					}	
				}	
				else
				{
...
var Stage=(function(_super){
	...
	*/
	...
	{
		// TODO ZF
		var screenHeightSub = this["__screenHeightSub"] ? this["__screenHeightSub"] : 0;
		var offestY = Math.round(screenHeightSub * 0.5);;
		offestY = this["__offestY"] ? this["__offestY"] : offestY;
		// TODO ZF
		screenHeight -= screenHeightSub;
	...
	*/
	__proto._syncInputTransform=function(){
		...
		if (Render.isConchApp){
			...
			inputElement.setPos(transform.x - 10, transform.y + 8);
			}else {
