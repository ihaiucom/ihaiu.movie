    ...
    (function (Common) {
        var RankMovieInfoPanel = /** @class */ (function (_super) {
            ...
            RankMovieInfoPanel.prototype.constructFromXML = function (xml) {
                ...
                this.m_btnBGClose.onClick(this, this.OnCloseClick);
            };
            ...
            RankMovieInfoPanel.prototype.UpdateView = function (movieData) {
                ...
                this.m_time.text = Games.TEXT.FilmLongTime.format(cfg.length);
                this.m_star.starNum = cfg.starNumber;
                this.m_mdiaEaluation.text = this.movieData.mediaCommend.toFixed(1);
                this.m_praise.text = this.movieData.publicPraise.toFixed(0);
                this.m_filmType.text = Game.config.property.GetPropertyName(cfg.type);
                this.UpdateFilmLabel();
            ...
            RankMovieInfoPanel.prototype.UpdateFilmLabel = function () {
                var cfg = Game.config.story.getConfig(this.movieData.movieId);
                if (cfg) {
                    var str = "";
                    for (var index = 0; index < cfg.tags.length; index++) {
                        var data = cfg.tags[index];
                        if (data) {
                            if (str != "") {
                                str += "/";
                            }
                            str += Game.config.property.GetPropertyName(data);
                        }
                    }
                    this.m_filmtype2.text = str;
                }
                else {
                    this.m_filmtype2.text = "";
                }
            };
