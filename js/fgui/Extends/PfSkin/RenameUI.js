...
(function (fgui) {
    ...
    (function (PfSkin) {
        var RenameUI = /** @class */ (function (_super) {
            ...
            // 	this.m_content.text = "";
            // 	this.m_okBtn.m_number.text = GlobalConfig.getValue1(GlobalKey.KEY_1010_ZiDingYiYi__MingChenXiaoHao)[1] + "";
            // 	this.getRandomNames();
            ...
            RenameUI.prototype.open = function (dialogMsg, caller, okFunc, closeFunc, okBtnTxt, defaultTxt, promptText, maxLen, url, cost, isClickButtonAutoClose, randomNames) {
                ...
                //最大字符数量
                this.m_content.m_title.input.maxChars = maxLen;
                this.charMaxLength = maxLen;
                //输入限制
                this.m_content.m_title.input.restrict = "a-zA-Z0-9\u4e00-\u9fa5\ ";
                //默认输入文本
                this.m_content.text = defaultTxt;
                //
                if (maxLen > 0) {
                    promptText = format(promptText, maxLen);
                }
                //提示信息
                this.m_content.m_title.promptText = promptText;
                this.focusTimeHandler = setTimeout(function () {
                    if (_this && _this.m_content && _this.m_content.m_title) {
                        //设置焦点
                        _this.m_content.m_title.input.focus = true;
                    }
                }, 1000);
                //回调函数
                this.okFunc = okFunc;
                ...
                //
                this.m_content.m_title.input.promptColor = "0x417295";
                if (isNullOrEmpty(url)) {
                    url = Game.config.avatar.GetBodyIconUrl(9991);
                }
                //图片
                ...
                //打开其他面板监听
                Game.event.add(GameEventKey.GameFrame_OpenMenu, this.onOpenMenu, this);
            ...
            RenameUI.prototype.onClickOkBtn = function () {
                if (validateOtherName(this.m_content.text, this.charMaxLength)) {
                    var enough = Games.ItemHelper.checkItemEnough3(this.cost);
                    if (enough) {
                        this.onClickNoCostBtn();
                    }
                    else {
                        Game.system.openItemWay(this.cost[0], this.cost[1]);
                    }
                }
            };
            /**
            ...
            RenameUI.prototype.onOpenMenu = function (menuId) {
                this.close();
            };
            /**
             * 关闭界面
            ...
            RenameUI.prototype.close = function () {
                Game.event.remove(GameEventKey.GameFrame_OpenMenu, this.onOpenMenu, this);
                clearTimeout(this.focusTimeHandler);
