    ...
    (function (Activity) {
        var ActivitySeven = /** @class */ (function (_super) {
            ...
            ActivitySeven.prototype.onRefrush = function () {
                var reward = [];
                var sevenConf = this.sevenConfig[Game.moduleModel.activity.curGear];
                for (var i = 0; i < sevenConf.special_reward.length; i++) {
                    reward.push(sevenConf.special_reward[i]);
                }
                for (var i = 0; i < sevenConf.reward.length; i++) {
                    reward.push(sevenConf.reward[i]);
                }
                Game.system.getRewardText(reward, TEXT.StoryGetReward, 0);
                // 签到完成，刷新状态
                this.m_rewardList.numItems = this.sevenConfig.length;
            };
