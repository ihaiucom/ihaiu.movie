    ...
    (function (WarMovie) {
        var WarMain = /** @class */ (function (_super) {
            ...
            WarMain.prototype.onWindowShow = function () {
                if (!Game.moduleModel.warData.isBattleOverCall) {
                ...
                Game.event.add(GameEventKey.GameEventKey_MakeFilm, this.subWindowToMain, this);
                Game.event.add(GameEventKey.GameEventKey_MakeFilmRename, this.subWindowToMain, this);
                Game.event.add(GameEventKey.GameEventKey_MakeFilmConfirmSetActor, this.subWindowToMain, this);
                Game.event.add(GameEventKey.GameEventKey_MakeFilmsetSelectType, this.subWindowToMain, this);
                Game.event.add(GameEventKey.GameEventKey_MakeFilmsetPublishMovie, this.subWindowToMain, this);
                Game.event.add(GameEventKey.GameEventKey_MakeFilmUpdateMovieState, this.subWindowToMain, this);
                Game.moduleModel.warData.sReGetLoginGameData.add(this.sReGetLoginGameData, this);
            };
            WarMain.prototype.onWindowHide = function () {
                ...
                Game.event.remove(GameEventKey.GameEventKey_MakeFilm, this.subWindowToMain, this);
                Game.event.remove(GameEventKey.GameEventKey_MakeFilmRename, this.subWindowToMain, this);
                Game.event.remove(GameEventKey.GameEventKey_MakeFilmConfirmSetActor, this.subWindowToMain, this);
                Game.event.remove(GameEventKey.GameEventKey_MakeFilmsetSelectType, this.subWindowToMain, this);
                Game.event.remove(GameEventKey.GameEventKey_MakeFilmsetPublishMovie, this.subWindowToMain, this);
                Game.event.remove(GameEventKey.GameEventKey_MakeFilmUpdateMovieState, this.subWindowToMain, this);
                Game.moduleModel.warData.sReGetLoginGameData.remove(this.sReGetLoginGameData, this);
                Game.system.hideRename();
            };
            // 重新登录
            WarMain.prototype.sReGetLoginGameData = function () {
                this.moduleWindow.closeAllSubwindowSelf();
                this.checkStatus();
            };
            ...
            WarMain.prototype.GameEventKey_TimeLimitRemind = function (timeLimit) {
                switch (timeLimit) {
                    case Games.TimeLimitType.NineToFiftyFive:
                        {
                            if (User.isNineToFiftyFive && Game.moduleModel.warData.moveData && Game.moduleModel.warData.moveData.state < EMovieState.result) {
            ...
            WarMain.prototype.startShowWaiting = function () {
                // GlobalModalWaiting.show(0, 200);
            };
            WarMain.prototype.endShowWaiting = function () {
            ...
            WarMain.prototype.shootingConfirmation = function () {
                if (!Game.moduleModel.street.canSign) {
                    if (Game.moduleModel.warData.realLicenceNum > 0) {
                        ...
                        if (Game.moduleModel.warData.licenceData && Game.moduleModel.warData.licenceData.dailyCount >= Game.config.global.getConfig(GlobalKey.Licence_Restore_Max).value) {
                            // 许可证领完
            ...
            WarMain.prototype.filmOverTip = function (status) {
                var _this = this;
                this.endShowWaiting();
                this.moduleWindow.closeLastSubwindowSelf();
                switch (status) {
                    ...
                    case EMovieState.rewardTip:
                        {
                            ...
                            Game.moduleModel.warData.jiesuanName = Game.moduleModel.warData.moveJieSuanData.name;
                            Game.system.getItemText(dtList, function () {
                                if (fansVal_1 > 0) {
                                    Game.system.addFansText(fansVal_1, Handler.create(_this, _this.triggerWarMainBack));
                                }
                                else {
                                    _this.triggerWarMainBack();
                                }
                            }, false);
            ...
            //弹出持续收益
            WarMain.prototype.triggerWarMainBack = function () {
                Game.system.alrtTextFull(format(Game.config.msg.getConfig(MsgKey.movie_off_line_tips).notice, Game.config.global.getConfig(GlobalKey.Popularize_Duration).value), Game.moduleModel.warData.jiesuanName, null, function () {
            ...
            WarMain.prototype.subWindowToMain = function () {
                ...
                this.endShowWaiting();
                if (Game.moduleModel.warData.moveData.state !== EMovieState.chooseType && Game.moduleModel.warData.moveData.state !== EMovieState.propaganda && Game.moduleModel.warData.moveData.state !== EMovieState.complete) {
                    this.moduleWindow.closeAllSubwindowSelf();
                }
                else {
                    this.moduleWindow.closeAllSubwindowSelf([this.moduleWindow.warFilmingSubwindow]);
                }
