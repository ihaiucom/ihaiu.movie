    ...
    (function (Produce) {
        ...
        var ProduceItem = /** @class */ (function (_super) {
            ...
            function ProduceItem() {
                ...
                _this.clientOutNum = 0; //客户端时间产量
                _this.clientProcessTime = 0; //客户端时间
                return _this;
            }
            ProduceItem.prototype.setInfo = function (produceId, grade) {
                ...
                this.m_produceDesc.m_descText.text = this.cfg.tip;
                this.m_produceDesc.m_descText.x = 2;
                ...
                //scroll text
                Laya.timer.clear(this, this.onScrollLongText);
                if (this.isLongText) {
                    Laya.timer.loop(30, this, this.onScrollLongText, null, true, true);
                }
            };
            /** 滚动较长描述文字 */
            ProduceItem.prototype.onScrollLongText = function () {
                this.m_produceDesc.m_descText.x -= 2;
                if (this.m_produceDesc.m_descText.x + this.m_produceDesc.m_descText.width < 0) {
                    this.m_produceDesc.m_descText.x = this.m_produceDesc.width;
                }
            };
            Object.defineProperty(ProduceItem.prototype, "isLongText", {
                get: function () {
                    return this.m_produceDesc.m_descText.width > this.m_produceDesc.width;
                },
                enumerable: true,
                configurable: true
            });
            ...
            ProduceItem.prototype.refresh = function () {
                ...
                var calcProduceLineNum = (this.data.processNum > this.timeProcessedlineNum ? this.timeProcessedlineNum : this.data.processNum);
                this.processedlineNum = calcProduceLineNum;
                for (var i = 0; i < calcProduceLineNum; i++) {
                    this.clientOutNum += this.perOutputNum();
                    this.clientProcessTime += this.totalTime;
                    if (this.checkOutputFull()) {
                        this.processedlineNum = i + 1;
                        //产量满了不再生产
                        break;
                    }
                }
            ...
            /** 重置客户端统计 */
            ProduceItem.prototype.resetClientData = function () {
                this.processedlineNum = 0;
                this.clientOutNum = 0;
                this.clientProcessTime = 0;
            };
            //-------------------------------
            // 服务器消息更新
            //-------------------------------
            ...
            ProduceItem.prototype.setOutputItemText = function () {
                if (this.checkOutputFull()) {
                    this.m_item.text = "[color=#e02f2b]" + (this.data.itemNum + this.clientOutNum) + "/" + this.maxStorage + "[/color]";
                }
                else {
                    this.m_item.text = "[color=#50e38a]" + (this.data.itemNum + this.clientOutNum) + "/" + this.maxStorage + "[/color]";
                }
            };
            ProduceItem.prototype.updateTime = function () {
                if (this.data.processNum - this.processedlineNum > 0 && !this.checkOutputFull()) {
            ...
            ProduceItem.prototype.onProcessTimeCutDown = function () {
                var curtimeProcessLineNum = Math.floor(Game.time.getSubSecondsPre(this.data.startTime + this.clientProcessTime) / this.totalTime);
                if (curtimeProcessLineNum > this.timeProcessedlineNum) {
                    ...
                    this.clientProcessTime += this.totalTime;
                    //生产线全部倒计时完
                    if (this.data.processNum - this.processedlineNum <= 0) {
                        this.stopTimeCutdown();
                    }
                    if (this.m_produceLine.numChildren > 0)
                        this.m_produceLine.removeChildAt(0);
                    ...
                    this.cutDownTime = this.totalTime - Game.time.getSubSecondsPre(this.data.startTime + this.clientProcessTime) % this.totalTime;
            ...
            ProduceItem.prototype.perOutputNum = function () {
                ...
                var baseOutput = Game.moduleModel.produce.getProduceLvEffect(outputSkill);
                //艺人基础产出鼓舞
            ...
            ProduceItem.prototype.getProcessTotalTime = function () {
                ...
                var skillOutput = this.data.produceSkillList[ProduceSkillType.Output - 1];
                var extTime = Game.moduleModel.produce.getProduceLvEffect(skillOutput, 1);
                var skillTime = this.data.produceSkillList[ProduceSkillType.Time - 1];
                var reduceTime = Game.moduleModel.produce.getProduceLvEffect(skillTime);
            ...
            ProduceItem.prototype.setMaxProcessLine = function () {
                ...
                if (pSkillProcess) {
                    var maxLine = Game.moduleModel.produce.getProduceLvEffect(pSkillProcess);
            ...
            Object.defineProperty(ProduceItem.prototype, "maxStorage", {
                ...
                get: function () {
                    ...
                    if (pSkillStorage) {
                        var save = Game.moduleModel.produce.getProduceLvEffect(pSkillStorage);
                        return save;
                    }
                    return 0;
                },
            ...
            ProduceItem.prototype.onTimeEndClick = function () {
                ...
                if (ItemHelper.getItemNum(Proto.SE.EItemId.diamond) < this.immediateProduceCost) {
                    Game.system.openItemWay(Proto.SE.EItemId.diamond, this.immediateProduceCost);
            ...
            ProduceItem.prototype.onGetItemClick = function () {
                ...
                var rewardList = new DTItemNum();
                rewardList.itemId = this.cfg.item;
                rewardList.itemNum = this.data.itemNum + this.clientOutNum;
                Games.Static.Instance.onResourceoutput([rewardList], "productEnd", this.produceId.toString());
            };
            //判断是否有奖励可领
            ProduceItem.prototype.isCanGetItem = function () {
                return this.data.itemNum + this.clientOutNum > 0;
            };
            //获取产出量
            ProduceItem.prototype.getOutItemNum = function () {
                if (this.isCanGetItem()) {
                    return this.data.itemNum + this.clientOutNum;
                }
                return 0;
            };
            //开始生产
            ...
            ProduceItem.prototype.checkOutputFull = function () {
                if (this.isFirstLine) {
                    var cfg = Game.config.produceSouvenir.getConfig(User.info.level);
                    return this.data.itemNum + this.clientOutNum >= cfg.max;
                }
                return this.data.itemNum + this.clientOutNum >= this.maxStorage;
            };
            //检查生产线是否已满
            ProduceItem.prototype.checkProcessLineFull = function () {
                var pSkillProcess = Game.moduleModel.produce.getProduceSkill(this.produceId, ProduceSkillType.Process);
                if (pSkillProcess) {
                    var line = Game.moduleModel.produce.getProduceLvEffect(pSkillProcess);
                    return this.data.processNum - this.processedlineNum >= line;
                }
                return false;
            };
            //检查立即完成生产条件
