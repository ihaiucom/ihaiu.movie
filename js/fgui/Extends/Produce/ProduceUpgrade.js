        ...
        var ProduceUpgrade = /** @class */ (function (_super) {
            ...
            ProduceUpgrade.prototype.setList = function () {
                ...
                for (var i = 0; i < this.data.produceSkillList.length; i++) {
                    ...
                    pSkillItem.setInfo(this.data.level, skill);
                    pSkillItem.m_btnUpgradeLock.onClick(this, this.onLockClick, [skill]);
            ...
            ProduceUpgrade.prototype.onLockClick = function (skill) {
                if (this.cfg) {
                    var produceLv = Game.moduleModel.produce.getProduceLvLimit(skill);
                    Game.system.toastText(Game.config.msg.getTxtFormat(MsgKey.produce_lv, this.cfg.name, produceLv));
                }
            };
            //升级单项
            ...
            ProduceUpgrade.prototype.checkCostEnough = function () {
                var pLevelCfg = Game.config.produceLevel.getConfig(this.data.level);
                var result = ItemHelper.getItemNum(pLevelCfg.cost.itemId) >= pLevelCfg.cost.itemNum;
                if (!result) {
                    Game.system.openItemWay(pLevelCfg.cost.itemId, pLevelCfg.cost.itemNum);
                }
                return result;
            };
