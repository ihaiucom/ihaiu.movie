    ...
    var ShopModel = /** @class */ (function (_super) {
            ...
            for (var index = 0; index < values.length; index++) {
                ...
                if (value != null) {
                    if (value.LimitType == 1) {
                        ...
                        if (Games.TimeHelper.IsWeek(Game.time.serverSeconds, 1)) {
                            value.hasBuy = 0;
                        }
                    }
                }
            ...
            for (var key in goodsList) {
                if (goodsList.hasOwnProperty(key)) {
                    ...
                    if (goods.ShopType == shoptype) {
                        if (goods.LimitType == 3 && goods.saleNums <= 0) {
                            //商品限购次数已经达到
                            continue;
                        }
                        list.push(goods);
        ...
        Object.defineProperty(ShopModel.prototype, "RedNum", {
            get: function () {
                ...
                // 	if (goods.Config.hasPeriodTime && goods.isPeriodTime && goods.saleNums > 0)
                // 	{
                // 		num++;
                // 	}
                // }
                return this.GetRedNumByType(1) + this.GetRedNumByType(2);
            },
            enumerable: true,
            configurable: true
        });
        ShopModel.prototype.GetRedNumByType = function (type) {
            var lastOpenTime;
            if (type == 1) {
                lastOpenTime = Game.localStorage.lastOpenShopSingleTime;
            }
            else {
                lastOpenTime = Game.localStorage.lastOpenShopSpecialTime;
            }
            var goodsList = this.GetGoodsByShopType(type);
            var num = 0;
            for (var index = 0; index < goodsList.length; index++) {
                var goods = goodsList[index];
                if (goods.isPeriodTime) {
                    var strTime = goods.Config.period.split('-');
                    if (strTime.length >= 2) {
                        var timeBegin = Games.TimeHelper.TimeReformat2(strTime[0]);
                        if (timeBegin > lastOpenTime) {
                            if (goods.saleNums > 0) {
                                num++;
                            }
                        }
