...
(function (Games) {
    ...
    var StoryModel = /** @class */ (function (_super) {
        ...
        StoryModel.prototype.resetSou = function () {
            if (Games.TimeHelper.IsSameDay(User.info.lastResetTime, Game.time.serverSeconds) == false) {
                User.info.findStoryTime = 0;
            }
            else {
                var currSecond = Game.time.serverSeconds;
                var nextSecond = Games.TimeHelper.NextDayZeroPointSecound();
        ...
        StoryModel.prototype.addStoryByProto = function (msg) {
            ...
            Games.Static.Instance.onUserorbit("script", format("{0}&{1}", data.isGold ? 1 : 0, msg.storyId), "unlock");
            if (data.config == null) {
        ...
        Object.defineProperty(StoryModel.prototype, "enoughItem", {
            /**
             * 是否足够
             */
            get: function () {
                var item = Game.config.blockLevel.getConfig(User.info.stall).storysearchcost;
                var enough = Games.ItemHelper.checkItemEnough(item);
                return enough;
            },
            enumerable: true,
            configurable: true
        ...
        Object.defineProperty(StoryModel.prototype, "isRed", {
            ...
            get: function () {
                ...
                var b = false;
                if (isUnlock) {
                    if ((this.isFindMax == false && this.remainTimers <= 0 && this.enoughItem) || Game.moduleModel.task.isRedStoryAchievement) {
                        b = true;
                    }
                }
                return b;
