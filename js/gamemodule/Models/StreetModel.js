...
(function (Games) {
    var StreetModel = /** @class */ (function (_super) {
        ...
        StreetModel.prototype.GetStreetNameTips = function (id) {
            var cfg = Game.config.blockLevel.getConfig(id);
            if (cfg) {
                return cfg.name_tips;
            }
            return "";
        };
        ...
        Object.defineProperty(StreetModel.prototype, "canReward", {
            get: function () {
                var hour = Game.config.global.getConfig(15).value;
                var date = Games.TimeHelper.TimeReformat(hour, 0);
                if (Game.time.serverMilliseconds > date.valueOf()) {
                    return date.valueOf() > User.info.lastGetStreetRewardTimt * 1000;
                }
                else {
                    return date.valueOf() - 24 * 60 * 60 * 1000 > User.info.lastGetStreetRewardTimt * 1000;
                }
            },
            enumerable: true,
            configurable: true
        ...
        Object.defineProperty(StreetModel.prototype, "hasCongratulateNums", {
            get: function () {
                ...
                var lastTime = Math.max(User.info.lastSendGroupRankListTime, User.info.lastSendGroupMovieRankListTime);
        ...
        StreetModel.prototype.IsNeedReset = function (lastResetTime, resetH, resetM, resetS) {
            ...
            var resetDate = Games.TimeHelper.TimeReformat(resetH, resetM, resetS);
            var resetTime = resetDate.getTime();
            // 如果当前还没到今日刷新时间
            if (!Games.TimeHelper.IsBeyondTimeInSameDay(Game.time.serverSeconds, resetH, resetM, resetS)) {
                resetTime = resetTime - 24 * 60 * 60 * 1000;
            }
            return lastResetTime * 1000 < resetTime;
        };
        ...
        Object.defineProperty(StreetModel.prototype, "groupRankCD", {
            get: function () {
                var hour = Game.config.global.getConfig(15).value;
                var date = Games.TimeHelper.TimeReformat(hour, 0, 0);
                if (Game.time.serverMilliseconds > date.valueOf()) {
                    return Math.floor(date.valueOf() / 1000 + 24 * 60 * 60 - Game.time.serverSeconds);
                }
                else {
                    return Math.floor(date.valueOf() / 1000 - Game.time.serverSeconds);
                }
            },
            enumerable: true,
            configurable: true
        });
        Object.defineProperty(StreetModel.prototype, "CityEnvoyMaxStreet", {
            get: function () {
                return this.GetStreetName(Game.config.blockLevel.CityEnvoyMaxStreet);
            },
            enumerable: true,
            configurable: true
        });
        ...
        Object.defineProperty(StreetModel.prototype, "RedNum", {
            get: function () {
                ...
                if (selfStreetData == null) {
                    return false;
                }
                return selfStreetData.isCurrent && Game.moduleModel.street.canReward;
            },
            enumerable: true,
            configurable: true
        });
