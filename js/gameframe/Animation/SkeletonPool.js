...
(function (Games) {
    ...
    var SkeletonPool = /** @class */ (function () {
         ...
         */
        SkeletonPool.prototype.requestShareItem = function (caller, onComplete, onError) {
            var item = this.getItem();
            if (item) {
                onComplete && onComplete.apply(caller, [item]);
            }
            else {
                // let curTime = laya.utils.Browser.now();
                // console.log("requestSkeleton -- time:" + (curTime - GameBoone.lastTime));
                // GameBoone.lastTime = curTime;
                this.animHelper.buildArmature(this.texturePath, this.skPath, caller, onComplete, onError);
                // GameBoone.animHelper.buildArmature(this.texturePath, this.skPath, caller, onComplete, onError);
            }
        };
        /**
         * 异步请求对象
